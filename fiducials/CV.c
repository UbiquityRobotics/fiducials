...
CV_Image CV_Image__pnm_read(const char * file_name) {
...
void CV_Image__pnm_write(CV_Image image, const char * file_name) {
    Unsigned size = String__size(file_name);
    assert (String__equal(file_name + size - 4, ".pnm"));
...
CV_Image CV_Image__tga_read(CV_Image image, const char * tga_file_name) {
    // Open *tga_in_file*:
...
void CV_Image__tga_write(CV_Image image, const char * file_name) {
    Unsigned channels = (Unsigned)image->nChannels;
